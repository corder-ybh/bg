...
class Captcha
{
    ...
    public function generate()
    {
        ...
        for ($i = 1; $i <= $this->width * $this->height * $this->pixelnum; $i++) {
            $pixecolor = imagecolorallocate($img, mt_rand(100, 150), mt_rand(0, 120), mt_rand(0, 255));
            imagesetpixel($img, mt_rand(0, $this->width - 1), mt_rand(0, $this->height - 1), $pixecolor);
        }
        // 7、输出图片
    ...
    private function getRandString()
    {
        ...
        shuffle($arr);
        // array_rand($arr, $num)
        // 从数组中取出$num个单元，并返回随机条目的一个或多个键。 它使用了伪随机数产生算法，所以不适合密码学场景，
